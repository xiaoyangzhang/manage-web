...
<script type="text/javascript">
    ...
    var childArr = [];
    if(window.parent.deptParams.length>0){
        childArr = window.parent.deptParams;
    }else{
        if(window.parent.articleDept&&window.parent.articleDept.length>0){
            for(var i=0;i<window.parent.articleDept.length;i++){
                childArr.push({
                    // questionnaireId:$("#questionnaireId").val()==undefined?0:$('#questionnaireId').val(),
                    dictDepartmentId:window.parent.articleDept[i].dictDepartmentId,
                    name:window.parent.articleDept[i].name
                });
            }
        }else{
            childArr = [];
