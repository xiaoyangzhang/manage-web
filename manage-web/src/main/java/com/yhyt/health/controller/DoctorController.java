...
public class DoctorController {
    ...
    public WebResult<DoctorVO> queryDoctorList(/*@RequestBody*/ DoctorQuery doctorQuery) {
        ...
        PageInfo doctors = restTemplate.getForObject(webPathConfiguration.getDoctorUrl()+"/doctorList?doctorQueryStr={doctorQueryStr}", PageInfo.class,JSON.toJSONString(doctorQuery));
...
//        return JSON.toJSONString(result);
    ...
    public WebResult<DoctorDiseaseVO> queryDoctorListForDisease(/*@RequestBody*/ DoctorQuery doctorQuery) {
        ...
        PageInfo doctors = restTemplate.getForObject(webPathConfiguration.getDoctorUrl()+"/disease/doctorList?doctorQueryStr={doctorQueryStr}", PageInfo.class,JSON.toJSONString(doctorQuery));
        result.setCount(doctors.getTotal());
        result.setList(doctors.getList());
        return result;
    }
...
//            Long currUserId = WebUtils.getCurrUserId(request);
            ...
            restTemplate.put(webPathConfiguration.getDoctorUrl()+"/updateDoctor?params={params}",null,JSON.toJSONString(doctor));
            return result;
    ...
    public String queryDoctorById(@PathVariable("id") long id,Model model) {
        DoctorVO doctor = restTemplate.getForObject(webPathConfiguration.getDoctorUrl()+"/toEditDoctor/"+id, DoctorVO.class);
        ...
        List titles = restTemplate.getForObject(webPathConfiguration.getSystemUrl()+"/dictionary/?dictCode="+ DictConstant.TITLE, List.class);
        model.addAttribute("titles",titles);
        List duties = restTemplate.getForObject(webPathConfiguration.getSystemUrl()+"/dictionary/?dictCode="+DictConstant.DUTY, List.class);
        model.addAttribute("duties",duties);
        List educations = restTemplate.getForObject(webPathConfiguration.getSystemUrl()+"/dictionary/?dictCode="+DictConstant.EDUCATION, List.class);
        //TODO 查询职称、职务、学历
        ...
        List list = restTemplate.getForObject(webPathConfiguration.getDoctorUrl()+"/getDepartmentList/" + doctor.getHospitalId(), List.class);
    ...
    public WebResult<DoctorVO> getDoctorBasicInfo(@PathVariable("id") Long id) {
        ...
        try {
            doctorVO = restTemplate.getForObject(webPathConfiguration.getDoctorUrl()+"/toEditDoctor/" + id, DoctorVO.class);
            //查询黑名单和反馈
            List blacklist = restTemplate.getForObject(webPathConfiguration.getSystemUrl()+"/getBlackList/" + id + "/"+2, List.class);
            if (!CollectionUtils.isEmpty(blacklist)) {

                doctorVO.setBlacklistList(blacklist);
            }
            List feedbacklist = restTemplate.getForObject(webPathConfiguration.getSystemUrl() + "/sysfeedback/feedbackList/query/" + id + "/" + 2, List.class);
            if (!CollectionUtils.isEmpty(blacklist)) {
    ...
    public WebResult<DoctorReviewVO> getDoctorReviewListPage( DoctorReviewQuery doctorReviewQuery) {
        ...
        PageInfo pageInfo = restTemplate.getForObject(webPathConfiguration.getDoctorUrl()+"/doctorReviewList?queryStr={queryStr}" , PageInfo.class, JSON.toJSONString(doctorReviewQuery));
        result.setCount(pageInfo.getTotal());
        result.setList(pageInfo.getList());
        return result;
    }

    @RequestMapping("/toDoctorReview/{id}/{state}")
    public String toDoctorReview(@PathVariable("id")long id,@PathVariable("state")String state, Model model,HttpServletRequest request){
        model.addAttribute("id",id);
    ...
    public WebResult<DoctorReviewVO> DoctorReviewInfo(@PathVariable("id")long id,HttpServletRequest request){
        WebResult<DoctorReviewVO> result = new WebResult<>();
        try {
            DoctorReviewVO doctorReviewVO = restTemplate.getForObject(webPathConfiguration.getDoctorUrl()+"/viewDoctorReviewInfo/" + id+"/"+WebUtils.getCurrUserName(request), DoctorReviewVO.class);
...
//            }
    ...
    public WebResult<String> releaseTask(HttpServletRequest request) {
        ...
        try {
            ...
            Integer integer = restTemplate.patchForObject(webPathConfiguration.getDoctorUrl()+"/releaseTask?operator={operator}", null, Integer.class, WebUtils.getCurrUserName(request));
            return result;
        } catch (RestClientException e) {
            result.setCode("500");
        }
    ...
    public WebResult<String> changeLockStateReview(@RequestBody DoctorReview doctorReview,HttpServletRequest request) {
        ...
        try {
            ...
            restTemplate.setRequestFactory(new HttpComponentsClientHttpRequestFactory());
            Integer integer = restTemplate.patchForObject(webPathConfiguration.getDoctorUrl()+"/changeLockStateReviewRecord", doctorHttpEntity, Integer.class);
            return result;
        } catch (RestClientException e) {
            result.setCode("500");
        }
    ...
    public WebResult<String> refuseApply(@RequestBody DoctorReview doctorReview){
        ...
        try {
            restTemplate.setRequestFactory(new HttpComponentsClientHttpRequestFactory());
            Integer integer = restTemplate.patchForObject(webPathConfiguration.getDoctorUrl()+"/refuseApply?params={params}", null, Integer.class, JSON.toJSONString(doctorReview));
            return result;
        } catch (RestClientException e) {
            result.setCode("500");
        }
    ...
    public WebResult<String> agreeApply(@PathVariable("id") long id){
        ...
        try {
            restTemplate.setRequestFactory(new HttpComponentsClientHttpRequestFactory());
            Integer integer = restTemplate.patchForObject(webPathConfiguration.getDoctorUrl()+"/agreeApply?id={id}", null, Integer.class, id);
            return result;
        } catch (RestClientException e) {
            result.setCode("500");
        }
...
//        }
        PageInfo pageInfo = restTemplate.getForObject(webPathConfiguration.getDoctorUrl()+"/getDoctorDisease?queryStr={queryStr}", PageInfo.class, JSON.toJSONString(doctorDiseaseQuery));
        result.setCount(pageInfo.getTotal());
        result.setList(pageInfo.getList());
        return result;
    }
...
//        }
        PageInfo pageInfo = restTemplate.getForObject(webPathConfiguration.getDoctorUrl()+"/getDoctorBlacklist?queryStr={queryStr}", PageInfo.class, JSON.toJSONString(doctorBlackQuery));
        result.setCount(pageInfo.getTotal());
        result.setList(pageInfo.getList());
        return result;
    }
    ...
    public WebResult<String> updatePassword(@RequestBody Doctor doctor) {
        ...
        try {
            restTemplate.setRequestFactory(new HttpComponentsClientHttpRequestFactory());
            Integer integer = restTemplate.patchForObject(webPathConfiguration.getDoctorUrl()+"/updatePassword?params={params}", null, Integer.class, JSON.toJSONString(doctor));
            return result;
        } catch (RestClientException e) {
            result.setCode("500");
        }
    ...
    public WebResult<String> addBlackList(String params) {
        ...
        try {
            //TODO 服务接口
            restTemplate.setRequestFactory(new HttpComponentsClientHttpRequestFactory());
            Integer integer = restTemplate.patchForObject(pathConfiguration.getDoctorUrl()+"/updatePassword?params={params}", null, Integer.class, params);
            return result;
        } catch (RestClientException e) {
            result.setCode("500");
        }
    ...
    public WebResult<String> operateBlacklist(@RequestBody SysBlacklistDTO sysBlacklistDTO,HttpServletRequest request){
        ...
        try {
            restTemplate.postForEntity(webPathConfiguration.getDoctorUrl()+"/blacklist/operate",httpEntity,Integer.class);
        } catch (RestClientException e) {
            result.setCode("500");
        }
        return result;
    ...
    public WebResult<DoctorVO> queryDeptDoctorList( DeptDoctorQuery deptDoctorQuery){
        WebResult<DoctorVO> result = new WebResult<>();
        try {
            PageInfo pageInfo = restTemplate.getForObject(webPathConfiguration.getDoctorUrl()+"/deptDoctorList/query?queryStr={}", PageInfo.class, JSON.toJSONString(deptDoctorQuery));
            result.setCount(pageInfo.getTotal());
            result.setList(pageInfo.getList());
            return result;
        } catch (RestClientException e) {
    ...
    public WebResult<String> deleteDeptDoctor(@PathVariable("id")long deptDoctorId){
        WebResult<String> result = new WebResult<>();
        try {
            restTemplate.delete(webPathConfiguration.getDoctorUrl()+"/deptDoctor/delete/"+deptDoctorId);
        } catch (RestClientException e) {
            result.setCode("500");
        }
        return result;
    ...
    public WebResult<String> dismissAdmin(@PathVariable("id")long deptDoctorId){
        ...
        try {
            restTemplate.setRequestFactory(new HttpComponentsClientHttpRequestFactory());
            restTemplate.patchForObject(webPathConfiguration.getDoctorUrl()+"/dismissAdmin/"+deptDoctorId,null,Integer.class);
        } catch (RestClientException e) {
            result.setCode("500");
        }
        return result;
    ...
    public WebResult<String> appointAdmin(@PathVariable("id")long deptDoctorId){
        ...
        try {
            restTemplate.setRequestFactory(new HttpComponentsClientHttpRequestFactory());
            restTemplate.patchForObject(webPathConfiguration.getDoctorUrl()+"/admin/appoint/"+deptDoctorId,null,Integer.class);
        } catch (RestClientException e) {
            result.setCode("500");
        }
        return result;
...
//        PageInfo pageInfo = restTemplate.getForObject(pathConfiguration.getDoctorUrl()+"/hospitalList?queryStr={queryStr}", PageInfo.class, JSON.toJSONString(hospitalQuery));
//        model.addAttribute("hospitalList",pageInfo.getList());
        ResponseEntity<DepartmentVO> forEntity = restTemplate.getForEntity(webPathConfiguration.getDoctorUrl() + "/dept/get/" + id, DepartmentVO.class);
        if (forEntity!=null){
    ...
    public WebResult<String> addDoctor(@RequestBody Doctor doctor,HttpServletRequest request){
        WebResult<String> result = new WebResult<>();
        Doctor forObject = restTemplate.getForObject(webPathConfiguration.getDoctorUrl() + "/doctor/get/" + doctor.getMobileNumber(), Doctor.class);
        if (forObject!=null){
            result.setCode("201");
        ...
        try {
            ResponseEntity<Integer> responseEntity = restTemplate.postForEntity(webPathConfiguration.getDoctorUrl()+"/createDepartDoctor", doctorHttpEntity, Integer.class);
        } catch (Exception e) {
            result.setCode("500");
        }
        return  result;
    ...
    public WebResult<DoctorExtVO> queryDoctorDiseaseListPage( DoctorQuery doctorQuery){
        ...
        try {
            PageInfo pageInfo = restTemplate.getForObject(webPathConfiguration.getDoctorUrl()+"/doctorDisease/query?queryStr={queryStr}", PageInfo.class, JSON.toJSONString(doctorQuery));
            result.setList(pageInfo.getList());
            result.setCount(pageInfo.getTotal());
        } catch (RestClientException e) {
            result.setCode("500");
    ...
    public WebResult<DoctorDiseaseVO> queryDiseaseDoctors( DoctorDiseaseQuery doctorQuery) {
        WebResult<DoctorDiseaseVO> result = new WebResult<>();
        try {
            PageInfo pageInfo = restTemplate.getForObject(webPathConfiguration.getDoctorUrl()+"/getDoctorDisease?queryStr={queryStr}", PageInfo.class,JSON.toJSONString(doctorQuery));
            result.setList(pageInfo.getList());
            result.setCount(pageInfo.getTotal());
        } catch (RestClientException e) {
            result.setCode("500");
    ...
    public WebResult<String> addDiseaseDoctor(@RequestParam("doctorDiseaseArr") String doctorDiseaseArr){
        ...
        try {
            ResponseEntity<Integer> responseEntity = restTemplate.postForEntity(webPathConfiguration.getDoctorUrl()+"/relateDoctorWithDisease", httpEntity, Integer.class);
        } catch (RestClientException e) {
            result.setCode("500");
        }
        return result;
...
//        List<DoctorDisease> doctorDiseaseList = JSON.parseArray(doctorDisease, DoctorDisease.class);
        ...
        try {
            ResponseEntity<String> responseEntity = restTemplate.postForEntity(webPathConfiguration.getDoctorUrl()+"/diseaseDoctor/add", httpEntity, String.class);
    ...
    public WebResult<String> delereDiseaseDoctor(@PathVariable("id")long id){
        WebResult<String> result = new WebResult<>();
        try {
            restTemplate.delete(webPathConfiguration.getDoctorUrl()+"/doctorDisease/delete/"+id);
        } catch (RestClientException e) {
            result.setCode("500");
        }
        return result;
    ...
    public WebResult<String> updateDiseaseDoctorSeq(@RequestParam("diseaseDoctorArr") String diseaseDoctorArr){
        ...
        try {
            restTemplate.setRequestFactory(new HttpComponentsClientHttpRequestFactory());
            Integer responseEntity = restTemplate.patchForObject(webPathConfiguration.getDoctorUrl()+"/doctorDisease/update", httpEntity, Integer.class);
        } catch (RestClientException e) {
            result.setCode("500");
        }
        return result;
    ...
    public WebResult<DoctorExtVO> queryFamousDoctors(@PathVariable("diseaseId")long id){
        ...
        List list = null;
        try {
            list = restTemplate.getForObject(webPathConfiguration.getDoctorUrl()+"/getDoctorsByDiseaseId/" + id, List.class);
            result.setList(list);
        } catch (RestClientException e) {
            result.setCode("500");
        }
    ...
    public WebResult<DoctorDiseaseVO> queryAddedDisease( DoctorDiseaseQuery doctorDiseaseQuery){
        WebResult<DoctorDiseaseVO> result = new WebResult<>();
        try {
            PageInfo pageInfo = restTemplate.getForObject(webPathConfiguration.getDoctorUrl()+"/addedDisease/query?queryStr={queryStr}", PageInfo.class, JSON.toJSONString(doctorDiseaseQuery));
            result.setCount(pageInfo.getTotal());
            result.setList(pageInfo.getList());
        } catch (RestClientException e) {
            result.setCode("500");
    ...
    public WebResult<String> addDoctorDisease(@PathVariable("diseaseId")long diseaseId,@PathVariable("doctorId")long doctorId){
        ...
        HttpEntity<DoctorDisease> httpEntity = new HttpEntity<>(doctorDisease, headers);
        try {
            restTemplate.postForEntity(webPathConfiguration.getDoctorUrl()+"/doctorDisease/add",httpEntity,Integer.class);
        } catch (RestClientException e) {
            result.setCode("500");
        }
        return  result;
...
//    public WebResult<String> deleteDoctorDisease
    ...
    public WebResult<SysLink> getH5List(String name,Integer pageIndex,Integer pageSize){
        ...
        try {
            Page forObject = restTemplate.getForObject(webPathConfiguration.getSystemUrl() + "/syslink/page?name={name}&pageIndex={pageIndex}&pageSize={pageSize}", Page.class, name, pageIndex, pageSize);
    ...
    public WebResult<String> addH5(@RequestBody SysLink sysLink,HttpServletRequest request){
        ...
        try {
            restTemplate.postForEntity(webPathConfiguration.getSystemUrl()+"/syslink/edit?content={content}&code={code}&url={url}&operatorId={operatorId}&id={id}",null,null,sysLink.getContent(),sysLink.getCode(),sysLink.getUrl(),sysLink.getOperatorId(),sysLink.getId());
        } catch (RestClientException e) {
            result.setCode("500");
        }
        return result;
    ...
    public String toEditH5(Model model,@PathVariable("id")Long id){
        SysLink forObject = restTemplate.getForObject(webPathConfiguration.getSystemUrl() + "/syslink/H5/get/" + id, SysLink.class);
   ...
   /* @RequestMapping("/H5/edit")
    ...
    public WebResult<String> editH5(@RequestBody SysLink sysLink,HttpServletRequest request){
        ...
        restTemplate.postForEntity(pathConfiguration.getSystemUrl()+"/syslink/edit",sysLink,null);
   ...
   public WebResult<String> deleteH5(@PathVariable("id")Long id){
       WebResult<String> result = new WebResult<>();
       try {
           restTemplate.delete(webPathConfiguration.getSystemUrl()+"/syslink/H5/delete/"+id);
       } catch (RestClientException e) {
           result.setCode("500");
       }
       return result;
   ...
   public String toQuestionnaireList(){
       return "/doctor/questionnaire-list";
   }

    /*随访问卷 新增*/
    @RequestMapping("/toQuestionnaireAdd")
    public String toQuestionnaireAdd(){
