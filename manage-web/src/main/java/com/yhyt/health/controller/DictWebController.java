...
public class DictWebController {
	...
	private static Logger logger = LoggerFactory.getLogger(DictWebController.class);
	 @Autowired
...
//		dictionary.setId(id);
	...
	public WebResult<DictCity> queryCities(@PathVariable("parentId")long id){
		...
		try {
			List list = restTemplate.getForObject(webPathConfiguration.getSystemUrl()+"/city/?parentId=" + id, List.class);
			result.setList(list);
	...
	public WebResult<DictDepartment> queryDeptCats(@PathVariable("parentCode")String code){
		...
		try {
			List list = restTemplate.getForObject(webPathConfiguration.getSystemUrl()+"/dictDepartmentList/" + code, List.class);
			result.setList(list);
		} catch (RestClientException e) {
			result.setCode("500");
		}
	...
	public WebResult<SysUpgrade>  upgradeListPage(UpgradeQuery upgradeQuery){
		...
		Page upgrades = restTemplate.getForObject(webPathConfiguration.getSystemUrl()+"/upgrade/list?queryStr={queryStr}", Page.class, JSON.toJSONString(upgradeQuery));
...
//		sysUpgrade.
		...
		try {
			ResponseEntity<Integer> responseEntity = restTemplate.postForEntity(webPathConfiguration.getSystemUrl()+"/upgrade/add", doctorHttpEntity, Integer.class);
		} catch (Exception e) {
			result.setCode("500");
		}
		return result;
	...
	public String toEditUpgrade(@PathVariable("id")long id, Model model){
		ResponseEntity<SysUpgrade> forEntity = restTemplate.getForEntity(webPathConfiguration.getSystemUrl()+ "/upgrade/get/" + id, SysUpgrade.class);
		if (forEntity!= null){
	...
	public WebResult<String>  editUpgrade(@RequestBody SysUpgrade sysUpgrade,HttpServletRequest request){
		...
		try {
			...
			sysUpgrade.setOperator(WebUtils.getCurrUserName(request));
			HttpEntity<SysUpgrade> doctorHttpEntity = new HttpEntity<>(sysUpgrade, headers);
			restTemplate.put(webPathConfiguration.getSystemUrl()+"/upgrade/update",doctorHttpEntity);
			return result;
		} catch (RestClientException e) {
			result.setCode("500");
		}
