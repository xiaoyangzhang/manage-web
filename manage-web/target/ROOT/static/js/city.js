/**
 * common.js By Beginner Emain:zheng_jinfan@126.com
...
(function($) {
		...
		$.fn.citys = function(parameter, getApi) {
			...
			return this.each(function() {
				...
				var _api = {};
				var $this = $(this);
				var $province = $this.find('select[name="'
						+ options.provinceField + '"]'), $city = $this
						.find('select[name="' + options.cityField + '"]'), $area = $this
						.find('select[name="' + options.areaField + '"]');
				$.ajax({
					url : options.dataUrl,
					type : 'GET',
					crossDomain : true,
					...
					success : function(data) {
							...
							for (code in data) {
								if (!(code % 1e4)) { // 获取所有的省级行政单位
									...
									if (options.required && !options.province) {
										if (options.city
												&& !(options.city % 1e4)) { // 省未填，并判断为直辖市
											options.province = options.city;
										} else {
											options.province = code;
										}
									} else if (data[code]
											.indexOf(options.province) > -1) {
										options.province = isNaN(options.province)
												? code
												: options.province;
									}
								} else {
										...
										if (!(code % 100)) {
											...
											if (options.required
												...
												options.city = code;
													...
													.indexOf(options.city) > -1) {
												options.city = isNaN(options.city)
														? code
														: options.city;
											}
											...
											if (options.city && c > 0
												...
												if (options.required
													...
													options.area = code;
														...
														.indexOf(options.area) > -1) {
													options.area = isNaN(options.area)
															? code
															: options.area;
												}
											}
						...
						var format = {
							...
							city : function() {
								...
								if (options.nodata == 'disabled') {
									...
									$city.css('display', $.isEmptyObject(city)
													? 'none'
													: '');
								}
								for (i in city) {
						...
						_api.getInfo = function() {
							var status = {
								...
								code : options.area || options.city
										|| options.province
							};
							return status;
						};
