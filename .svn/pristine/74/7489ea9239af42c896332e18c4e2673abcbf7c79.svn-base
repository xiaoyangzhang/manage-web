...
<div style="margin:0px; background-color: white;">
    <blockquote class="layui-elem-quote">
        <form class="layui-form" style="">
            <input type="hidden" name="id" id="deptId" value="${id}" />
...
<script >
    layui.config({
        ...
        paging.init({
            openWait: true,
            url: '<%=staticPath%>/doctor/doctorList', //地址
            elem: '#content', //内容容器
            params: { //发送到服务端的参数
...
//        });
    ...
    var getSelectedDoctors=function () {
        ...
        $('#content').children('tr').each(function() {
            var $that = $(this);
//            var $cbx = $that.children('td:first-child').find('input[]');
            var $cbx=$that.children('td').eq(0).children('input[type=checkbox]')[0];
            if ($cbx.checked){
                var deptDoctor={
                    departmentId:$('#deptId').val(),
                    doctorId:$($cbx).data("id")
                };
                deptDoctorArr.push(deptDoctor);
            }

        });
        console.log('你选择的名称有：' + deptDoctorArr);
        return deptDoctorArr;
    }
