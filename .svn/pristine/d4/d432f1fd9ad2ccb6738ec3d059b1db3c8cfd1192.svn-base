...
<div class="layui-whole">
	<div class="layui-whole-con" style="padding: 0px 15px;">
		...
		<table class="layui-table" style="margin:0 0 10px 0;">
			...
			<tr>
				<td>付款时间</td>
				<td><fmt:formatDate value="${detail.payTime}" pattern="yyyy-MM-dd HH:mm" type="date"/></td>
				<td>退款时间</td>
				<td><fmt:formatDate value="${detail.refundTime}" pattern="yyyy-MM-dd HH:mm" type="date"/></td>
			</tr>
			</tbody>
		</table>
		...
		<div style="padding: 10px 0;">
			<p>任务说明: <span style="color: #f00">开启任务之前请与患者确认病情信息以及服务进程，与合作科室沟通启动任务；<b>退单或审核通过才能结束任务</b></span></p>
		</div>
		</fieldset>
