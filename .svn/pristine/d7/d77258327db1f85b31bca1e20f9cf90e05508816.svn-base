...
layui.define(['element', 'common', 'paging', 'form'], function (exports) {
    ...
    function getTpl(options) {
        ...
        var tpl = '{{# if(d.list!=undefined && d.list!=null && d.list.length>0 ){ }}';
        tpl += '{{# layui.each(d.list, function(index, item){ }}';
        var tds = '';
        for (var i = 0; i < columns.length; i++) {
        	tds += '{{# if(!item.' + columns[i].field + '||null==item.' + columns[i].field + '||"NULL"==item.'+columns[i].field +') item.' + columns[i].field + '= ""; }}';
            tds += '<td class="emoji" data-field="' + columns[i].field + '">{{ item.' + columns[i].field + ' }}</td>';
        }
        if (options.checkbox) {
        ...
        tpl += '{{# }); }}';
        tpl += '{{# }else{ }}';
        var colLength = options.checkbox ? columns.length + 2 : columns.length+1;
        tpl += '<td  colspan="' + colLength + '" align="center">暂无数据.</td>';
        tpl += '{{# } }}';
        return tpl;
    }
