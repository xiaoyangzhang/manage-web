...
<script >
    ...
    layui.config({
        ...
        btable.set({
            openWait: true,//开启等待框
            elem: '#diseaseAdded',
            url: '<%=staticPath%>/doctor/addedDisease/query', //数据源地址
            pageSize: 500,//页大小
            params:{
            ...
            columns:[{
                ...
                format: function (val,obj) {
                    var html ="";
                    if(obj)
                        html = '<input type="button" data-field="'+obj.diseaseId+'"  value="移除" lay-filter="remove" data-action="remove" data-id="' + val + '" class="layui-btn layui-btn-mini" /> ';
                    else
                ...
                $elem.children('tr').each(function () {
                    $(this).children('td:last-child').children('input').each(function () {
                        ...
                        $that.on('click', function () {
                            $.ajax({
                               url:'<%=staticPath%>/doctor/diseaseDoctor/delere/'+id,
                               type:"get",
                               dataType:'json',
