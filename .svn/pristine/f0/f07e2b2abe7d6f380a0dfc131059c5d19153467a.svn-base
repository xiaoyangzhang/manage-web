layui.config({
	...
	$ = layui.jquery, layer = layui.layer, navbar = layui.navbar(), tab = layui
			.tab({
	...
	$menu.find('li.layui-nav-item').each(function() {
				...
				$this.on('click', function() {
							 ...
							 * navbar.set({ elem: '#side', data: data });
							 * navbar.render(); navbar.on('click(side)',
							 * function(data) { tab.tabAdd(data.field); }); });
							 */
							// ------------栗子结束--------------
							// 第二：设置url
							// ---------这是第二个栗子----------
							/*
							 * navbar.set({ elem: '#side', url:
							 * '/api/xxx?moduleId='+id }); navbar.render();
